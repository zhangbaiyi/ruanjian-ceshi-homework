...
export default {
  ...
  methods: {
    ...
    beforeFileUpload(file) {
      ...
      if (!isCSV) {
        this.$message.error("仅支持上传.csv文件。");
      }
      if (!isLt500K) {
        this.$message.error("上传文件的大小不能超过500KB。");
      }
      return isCSV && isLt500K;
    },
    handle_file(res) {
      console.log(res);
      this.result = res;
    ...
    getFileSrc() {
      ...
      this.$http({
        url: "api/sales/download/" + this.file.name,
        method: "GET",
      })
        .then((res) => {
          let file = res.data;
    ...
    init_chart() {
      this.chartInstance = this.$echarts.init(
        document.getElementById("com-chart")
      );
    },
    update_chart() {
      const trueValue = this.result.True;
      const falseValue = this.result.False;
      const piePatternSrc =
